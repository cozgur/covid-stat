...
struct TotalDataView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                TotalDataCard(number: String(format: "%.2f", totalData.fatalityRate), name: "Deaths %",color: .purple)
                TotalDataCard(number: totalData.recovered.formatNumber(), name: "Recovered",color: .green)
                TotalDataCard(number: String(format: "%.2f", totalData.recoveredRate), name: "Recovery %", color: .blue)
            }
